#include "Arduino.h"
///////////////////
//  Pv router by Cyril Poissonnier 2022 
...
String getconfig() {
  ...
  configweb = VERSION + ";" + stringbool(config.mqtt) +  ";" + config.mqttserver + ";"  + String(config.Publish) + ";" + config.IDX + ";" +  String(config.IDXdimmer) + ";"  + config.num_fuse + ";"  + config.delta + ";" + config.deltaneg +";" +config.dimmer+";"+config.cosphi+";"+stringbool(config.autonome)+";"+config.facteur+";"+config.resistance;
...
// init port
  ...
  if(!LittleFS.exists("/index.html")){
    Serial.println("Attention fichiers LittleFS non chargé sur l'ESP, ça ne fonctionnera pas.");  
    logs += "Attention fichiers LittleFS non chargé sur l'ESP, ça ne fonctionnera pas.";
    }
  ...
  server.on("/",HTTP_ANY, [](AsyncWebServerRequest *request){
    ...
    else {request->send_P(200, "text/plain", "fichiers LittleFS non présent sur l ESP. " ); }
  });
...
server.on("/doc.txt", HTTP_ANY, [](AsyncWebServerRequest *request){
    request->send(LittleFS, "/doc.txt", "text/plain");
  });
...
server.on("/get", HTTP_ANY, [] (AsyncWebServerRequest *request) {
  ...
  mqtt(String(config.IDXdimmer),"0");  }

  /// reset dimmer
  String baseurl; 
  dimmer_power=0;
  baseurl = "/?POWER=" + String(dimmer_power) ; http.begin(domotic_client,config.dimmer,80,baseurl);   
   //httpCode = 
   http.GET();  
   http.end();
  delay(5000); 
...
int mesure_resistance () {
 // mise à 0 dimmer 
  ...
  http.begin(domotic_client,config.dimmer,80,baseurl);   
...
boolean SendToDomotic(String Svalue){
  ...
  if ( config.mqtt == 1 ) {     mqtt(String(config.IDX),Svalue);  }
  Serial.println(baseurl);
  
  if ( config.autonome == 1 && change == 1   ) {  baseurl = "/?POWER=" + String(dimmer_power) ; http.begin(domotic_client,config.dimmer,80,baseurl);   
    http.GET();
    http.end(); 
    if ( config.mqtt == 1 ) { mqtt(String(config.IDXdimmer), String(dimmer_power));  }
